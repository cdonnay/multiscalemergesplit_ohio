...
def loopErasedWalk(graph, rng, v1 = None, v2 = None):
    ...
    if isinstance(graph, nx.classes.multigraph.MultiGraph):
        while v not in v2:
            options = sorted(list(graph.neighbors(v)))
            weights = [graph.number_of_edges(v, u) for u in options]
            invCumSum = 1.0/float(sum(weights))
            weights = [w*invCumSum for w in weights] 
            # print(options, weights, v)
...
def findEdgeCutWeights(root, tree, treePop, distFromRoot, graph, state, 
    ...
    for edge in sortedEdges:
        ...
        if leafEdge_huh:
            try:
                ...
                print("edge", edge)
                print("deg", [(n, tree.degree(n)) for n in edge])
...
def edgeCuts(tree, treePop, graph, state, boolop = operator.and_, 
             retRoot = False):
    ...
    possibleEdgeCuts, edgeWeights = findEdgeCutWeights(root, tree, treePop, 
                                                       distFromRoot, graph, 
                                                       state, boolop)
    if not retRoot:
        return possibleEdgeCuts, edgeWeights
